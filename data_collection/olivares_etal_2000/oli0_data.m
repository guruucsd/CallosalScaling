oli0_path = fileparts(which(mfilename));
data_file = fullfile(oli0_path, 'oli0_data.mat');

consistency_check = false;

if ~exist(data_file, 'file')
    [img, orig_img] = scrub_image('img/Fig3_greendot.png', 0, 'g', 255);

    % Get the (colored) datapoints
    o_fig3_species = {'Rat','Rabbit','Cat','Dog','Cow','Horse','Human'};
    o_species_n = [14 6 19 20 19 17 40];

    [data_ypix,data_xpix] = find(~img);
    if length(data_ypix)~=length(o_fig3_species), error('?'); end;
    ...
    o_fig3_brain_weights = 10.^((data_xpix - xticks(1))/mean(diff(xticks))*0.5)
    o_fig3_ccas = 10.^((yticks(end)-data_ypix)/mean(diff(yticks))*1)
