    ...
    if ~exist('lrb_fig3_data','var') || any(~all(sum(lrb_fig3_data,3)))
        ...
        for ai=1:length(lrb_fig3_ages)
            for si=1:length(lrb_fig3_sectors)
                ...
                lrb_fig3_data(ai,si,:) = lrb_process_image(fullfile(LRB_img_dirpath, fn), lrb_fig3_xbar_vals, lrb_fig3_ytick_vals, 0, true);
            end;
        end;
    end;
    ...
    if ~exist('lrb_fig15_data','var') || any(~all(sum(lrb_fig15_data,3)))
        ...
        for ai=1:length(lrb_fig15_ages)
            for si=1:length(lrb_fig15_sectors)
                ...
                lrb_fig15_data(ai,si,:) = lrb_process_image(fullfile(LRB_img_dirpath, fn), lrb_fig15_xbar_vals, lrb_fig15_ytick_vals, lrb_fig15_rotang, true);
            end;
        end;
    end;
